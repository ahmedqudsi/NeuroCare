...
const pageStaticText = {
  title: "AI Speech Rehabilitation",
  description: "Enter the text of your speech below. Our AI will analyze it and provide structured feedback on pronunciation, fluency, and clarity.",
  labelText: "Your Speech Text",
  placeholderText: "Type or paste your speech here...",
  buttonLoading: "Analyzing...",
  buttonDefault: "Get Feedback",
  errorTitle: "Error",
  ...
  validationMinChars: "Please enter at least 10 characters of speech.",
  validationMaxChars: "Speech text cannot exceed 1000 characters."
};

const formSchema = z.object({
  speechText: z.string().min(10, { message: pageStaticText.validationMinChars }).max(1000, {message: pageStaticText.validationMaxChars}),
});

type FormData = z.infer<typeof formSchema>;

// dictionary prop removed from SpeechAnalysisClientProps
export function SpeechAnalysisClient() {
  ...
  const form = useForm<FormData>({
    resolver: zodResolver(formSchema), // Use the schema with static messages
    defaultValues: {
      speechText: '',
    },
  });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CardContent className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel htmlFor="speechText" className="text-base">{pageStaticText.labelText}</FormLabel>
                  <FormControl>
                    <Textarea
                      id="speechText"
                      placeholder={pageStaticText.placeholderText}
                      rows={6}
          ...
          <CardFooter className="flex flex-col items-stretch gap-4">
            <Button type="submit" disabled={isLoading} className="w-full shadow-md hover:shadow-lg transition-shadow">
              {isLoading ? (
                <>
                  ...
                  {pageStaticText.buttonLoading}
                </>
              ) : (
                <>
                  <Sparkles className="mr-2 h-4 w-4" />
                  {pageStaticText.buttonDefault}
                </>
              )}
            </Button>
            ...
            {error && (
              <Alert variant="destructive">
                ...
                <AlertTitle>{pageStaticText.errorTitle}</AlertTitle>
              ...
              <Card className="mt-4 bg-secondary">
                <CardHeader>
                  <CardTitle className="text-lg flex items-center">
                    ...
                    {pageStaticText.feedbackTitle}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4 text-sm">
                    <div>
                      <h4 className="font-semibold text-base mb-1">{pageStaticText.overallAssessmentTitle}</h4>
                      <p className="whitespace-pre-wrap text-muted-foreground">{feedback.overallAssessment}</p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-base mb-1">{pageStaticText.pronunciationTitle}</h4>
                      <p className="whitespace-pre-wrap text-muted-foreground">{feedback.pronunciationFeedback}</p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-base mb-1">{pageStaticText.fluencyTitle}</h4>
                      <p className="whitespace-pre-wrap text-muted-foreground">{feedback.fluencyFeedback}</p>
                    </div>
                    <div>
                      <h4 className="font-semibold text-base mb-1">{pageStaticText.clarityTitle}</h4>
                      <p className="whitespace-pre-wrap text-muted-foreground">{feedback.clarityFeedback}</p>
                    </div>
                    {feedback.suggestions && feedback.suggestions.length > 0 && (
                      <div>
                        <h4 className="font-semibold text-base mb-1">{pageStaticText.suggestionsTitle}</h4>
